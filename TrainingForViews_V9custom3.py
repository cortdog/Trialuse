...
def fetch_data(batch_size, train=True, generator=False):
    ...
    for i in range(batch_size):
        ...
        with tf.Session() as sess:
            for k, img in enumerate(tar_array):
                img_resized=tf.random_crop(img, size=(450, 600, 3))
                img_done=sess.run(img_resized)
                img_done=np.expand_dims(img_done, axis=0)
                if k==0: tar_array_sub=img_done
                else: tar_array_sub=np.concatenate((tar_array_sub, img_done))
            tar_array=tar_array_sub
        '''
...
from keras.layers import TimeDistributed, Dense, SimpleRNN, Input, Flatten, Convolution2D, InputLayer, Reshape, Conv2D, MaxPooling2D, Dropout, BatchNormalization, AveragePooling2D
...
def made_model():
    ...
    model.add(InputLayer(input_shape=(50, 450, 600, 3)))
    model.add(Reshape((5, 10, 450, 600, 3)))
    model.add(TimeDistributed(TimeDistributed(AveragePooling2D(pool_size=(2,2)))))
    ...
    model.add(TimeDistributed(TimeDistributed(Conv2D(kernel_size=5, strides=1, filters=10, padding='same',
    ...
    model.add(TimeDistributed(TimeDistributed(Dense(64, activation='relu'))))
    
    model.add(TimeDistributed(SimpleRNN(128, return_sequences=False, stateful=False)))
